...
if (!defined('XML_HTMLSAX')) {
    define('XML_HTMLSAX', dirname(__FILE__));
}

$sPathSeparator = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') ? ';' : ':';
$sIncludePath = ini_get('include_path');
if (!$sIncludePath) {
    $sIncludePath = '.';
}
$sNewPath = VDAEMON_USE_BUNDLED_PEAR ? '.' : $sIncludePath;
$sNewPath .= $sPathSeparator . XML_HTMLSAX;
ini_set('include_path', $sNewPath);

require_once('PEAR.php');
require_once('HTMLSax/XML_HTMLSax_States.php');
require_once('HTMLSax/XML_HTMLSax_Decorators.php');
